...
/**
	 ...
	 * @param {function} callback
	 * @returns {void}
	 */show(message,type,callback){const t=document.getElementsByClassName(this.options.position)[0],e=document.createElement("div");e.className="alert-card "+type,e.innerHTML+=svgs[type],e.options={...this.options,message:message,type:type,yPos:-1<this.options.position.indexOf("top")?"top":"bottom",inFocus:!1},this._setContent(e),this._setIntroAnimation(e),this._bindEvents(e),this._autoDestroy(e,callback),t.appendChild(e)}_setContent(card){const t=document.createElement("div");t.className="text-group",card.options.title&&(t.innerHTML=`<h4>${card.options.title}</h3>`),t.innerHTML+=`<p>${card.options.message}</p>`,card.appendChild(t)}
/**
...
/**
	 * @param {AlertCard} card
	 * @returns {void}
	 */_destroy(card,callback){card.style.setProperty("margin-"+card.options.yPos,`-${card.offsetHeight}px`),card.style.setProperty("opacity","0"),setTimeout(()=>{card.remove(),"function"==typeof callback&&callback()},500)}_setup(){const t=document.createElement("div");t.className="alert-container";for(const e of[0,1]){const s=document.createElement("div");s.className="alert-row";for(const i of[0,1,2]){const o=document.createElement("div");o.className="alert-col "+alertPositionIndex[e][i],s.appendChild(o)}t.appendChild(s)}document.body.appendChild(t)}}
...
/**
	 ...
	 * @param {function} callback
	 * @returns {void}
	 */show(type,message,callback){const t=document.createElement("dialog");t.classList.add("confirm-container");let e=new Shim(!0,()=>{this._close(t,callback,!1)});t.innerHTML+=`<p>${message}</p>`,this._addButtons(t,type,e,callback),e.show(),document.body.appendChild(t)}_close(container,callback,callbackValue){container.remove(),"function"==typeof callback&&callback(callbackValue)}_addButtons(container,type,shim,callback){let t=[];if("yesno"===type){const i=document.createElement("button"),o=(i.innerText="No",i.classList.add("cancel-button"),i.addEventListener("click",e=>{e.preventDefault(),e.stopPropagation(),shim.hide(!1),this._close(container,callback,!1)}),document.createElement("button"));o.innerText="Yes",o.classList.add("action-button"),o.addEventListener("click",e=>{e.preventDefault(),e.stopPropagation(),shim.hide(!1),this._close(container,callback,!0)}),t.push(i),t.push(o)}else{const a=document.createElement("button");a.innerText="Close",a.classList.add("action-button"),a.addEventListener("click",e=>{e.preventDefault(),e.stopPropagation(),shim.hide(!1),this._close(container,callback)}),t.push(a)}const s=document.createElement("div");s.classList.add("button-row"),t.forEach(button=>{s.appendChild(button)}),container.appendChild(s)}}
/**
...
/**
	 ...
	 * @returns {void}
	 */addValidatorFunc(f){this.validatorFunc=f}_onCancelClick(){this.hide(),this.dispatchEvent(new CustomEvent("cancel"))}_onConfirmClick(){let e={};if(this.querySelectorAll("input, select, textarea").forEach(el=>{let t="";el.hasAttribute("name")?t=el.getAttribute("name"):el.hasAttribute("id")&&(t=el.getAttribute("id")),e[t]=el.value}),this.validatorFunc){var{validationErrors:t,isValid:s}=this.validatorFunc(e);if(!s)return void this.dispatchEvent(new CustomEvent("validation-failed",{detail:{result:e,validationErrors:t}}))}this.hide(),this.dispatchEvent(new CustomEvent("confirm",{detail:e}))}_renderWindow(){this.windowEl=document.createElement("div"),this.windowEl.classList.add("prompter"),this.windowEl.setAttribute("role","dialog"),this.windowEl.setAttribute("aria-modal","true"),this.windowEl.setAttribute("aria-label","Prompt"),this.windowEl.style.width=this.width,this.windowEl.style.height=this.height,this.windowEl.innerHTML=`
			<slot name="title"></slot>
			<slot name="body"></slot>
		`}_clearAllInputs(){this.querySelectorAll("input, select, textarea").forEach(el=>{el.value=""})}}customElements.get("prompter-ui")||customElements.define("prompter-ui",Prompter);export{AlertPosition,Alerter,Confirmer,Prompter};