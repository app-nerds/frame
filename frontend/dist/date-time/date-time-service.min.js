 ...
 * @param {Date} dt
 * @returns {string}
 */function format1(dt){var{year:dt,month:t,day:e,hour:d,minute:a,second:r}=breakDownDate(dt);return`${zeroPad(dt)}-${zeroPad(t)}-${zeroPad(e)}T${zeroPad(d)}:${zeroPad(a)}:${zeroPad(r)}Z`}
/**
 * @param {Date} dt
 * @returns {string}
 */function format2(dt){var{year:dt,month:t,day:e,hour:d,minute:a,second:r}=breakDownDate(dt);return`${zeroPad(dt)}-${zeroPad(t)}-${zeroPad(e)}T${zeroPad(d)}:${zeroPad(a)}:`+zeroPad(r)}
/**
 * @param {Date} dt
 * @returns {string}
 */function format3(dt){var{year:dt,month:t,day:e,hour:d,minute:a,second:r}=breakDownDate(dt);return`${zeroPad(dt)}-${zeroPad(t)}-${zeroPad(e)} ${zeroPad(d)}:${zeroPad(a)}:`+zeroPad(r)}
/**
 * @param {Date} dt
 * @returns {string}
 */function format4(dt){var{year:dt,month:t,day:e,hour:d,minute:a}=breakDownDate(dt);return`${zeroPad(dt)}-${zeroPad(t)}-${zeroPad(e)} ${zeroPad(d)}:`+zeroPad(a)}
/**
 * @param {Date} dt
 * @returns {string}
 */function format5(dt){let{year:t,month:e,day:d,hour:a,minute:r,second:o}=breakDownDate(dt),m="AM";return 12<a&&(m="PM",a-=12),0===a&&(a=12),`${zeroPad(e)}/${zeroPad(d)}/${zeroPad(t)} ${zeroPad(a)}:${zeroPad(r)}:${zeroPad(o)} `+m}
/**
 * @param {Date} dt
 * @returns {string}
 */function format6(dt){let{year:t,month:e,day:d,hour:a,minute:r}=breakDownDate(dt),o="AM";return 12<a&&(o="PM",a-=12),0===a&&(a=12),`${zeroPad(e)}/${zeroPad(d)}/${zeroPad(t)} ${zeroPad(a)}:${zeroPad(r)} `+o}
/**
 * @param {Date} dt
 * @returns {string}
...
/(?<month>\d{2})\/(?<day>\d{2})\/(?<year>\d{4})/];for(let t=0;t<e.length;t++){var d=dt.match(e[t]);if(d){var a=parseInt(d.groups.year)||(new Date).getFullYear(),r=parseInt(d.groups.month)||(new Date).getMonth(),o=parseInt(d.groups.day)||(new Date).getDate();let t=parseInt(d.groups.hour)||0;var m=parseInt(d.groups.minute)||0,n=parseInt(d.groups.second)||0;let e=d.groups.meridian||"";return""!==e&&"pm"===e.toLowerCase()&&t<12&&(t+=12),new Date(a,r-1,o,t,m,n)}}throw new Error("no pattern match for "+dt)}export{DateFormats,formatDateTime,parseDateTime};