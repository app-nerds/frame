...
/**
	 * @param {AlertCard} card
	 * @returns {void}
	 */_destroy(card,callback){card.style.setProperty("margin-"+card.options.yPos,`-${card.offsetHeight}px`),card.style.setProperty("opacity","0"),setTimeout(()=>{card.remove(),"function"==typeof callback&&callback()},500)}_setup(){const t=document.createElement("div");t.className="alert-container";for(const e of[0,1]){const s=document.createElement("div");s.className="alert-row";for(const o of[0,1,2]){const a=document.createElement("div");a.className="alert-col "+alertPositionIndex[e][o],s.appendChild(a)}t.appendChild(s)}document.body.appendChild(t)}}
/*
 ...
 * Copyright © 2023 App Nerds LLC
 */
/** @typedef {object & { callback: Function }} ConfirmOptions */class Confirmer{constructor(){}
/**
...
/**
	 ...
	 * @param {function} callback
	 * @returns {void}
	 */show(type,message,callback){const t=document.createElement("dialog");t.classList.add("confirm-container");let e=new Shim(!0,()=>{this._close(t,callback,!1)});t.innerHTML+=`<p>${message}</p>`,this._addButtons(t,type,e,callback),e.show(),document.body.appendChild(t)}_close(container,callback,callbackValue){container.remove(),"function"==typeof callback&&callback(callbackValue)}_addButtons(container,type,shim,callback){let t=[];if("yesno"===type){const o=document.createElement("button"),a=(o.innerText="No",o.classList.add("cancel-button"),o.addEventListener("click",e=>{e.preventDefault(),e.stopPropagation(),shim.hide(!1),this._close(container,callback,!1)}),document.createElement("button"));a.innerText="Yes",a.classList.add("action-button"),a.addEventListener("click",e=>{e.preventDefault(),e.stopPropagation(),shim.hide(!1),this._close(container,callback,!0)}),t.push(o),t.push(a)}else{const c=document.createElement("button");c.innerText="Close",c.classList.add("action-button"),c.addEventListener("click",e=>{e.preventDefault(),e.stopPropagation(),shim.hide(!1),this._close(container,callback)}),t.push(c)}const s=document.createElement("div");s.classList.add("button-row"),t.forEach(button=>{s.appendChild(button)}),container.appendChild(s)}}export{AlertPosition,Alerter,Confirmer};