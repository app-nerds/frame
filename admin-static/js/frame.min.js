...
/**
	 * @param {AlertCard} card
	 * @returns {void}
	 */_destroy(card,callback){card.style.setProperty("margin-"+card.options.yPos,`-${card.offsetHeight}px`),card.style.setProperty("opacity","0"),setTimeout(()=>{card.remove(),"function"==typeof callback&&callback()},500)}_setup(){const e=document.createElement("div");e.className="alert-container";for(const t of[0,1]){const n=document.createElement("div");n.className="alert-row";for(const r of[0,1,2]){const a=document.createElement("div");a.className="alert-col "+alertPositionIndex[t][r],n.appendChild(a)}e.appendChild(n)}document.body.appendChild(e)}}
...
/**
	 ...
	 * @param {function} callback
	 * @returns {void}
	 */show(type,message,callback){const e=document.createElement("dialog");e.classList.add("confirm-container");let t=new Shim(!0,()=>{this._close(e,callback,!1)});e.innerHTML+=`<p>${message}</p>`,this._addButtons(e,type,t,callback),t.show(),document.body.appendChild(e)}_close(container,callback,callbackValue){container.remove(),"function"==typeof callback&&callback(callbackValue)}_addButtons(container,type,shim,callback){let t=[];if("yesno"===type){const r=document.createElement("button"),a=(r.innerText="No",r.classList.add("cancel-button"),r.addEventListener("click",e=>{e.preventDefault(),e.stopPropagation(),shim.hide(!1),this._close(container,callback,!1)}),document.createElement("button"));a.innerText="Yes",a.classList.add("action-button"),a.addEventListener("click",e=>{e.preventDefault(),e.stopPropagation(),shim.hide(!1),this._close(container,callback,!0)}),t.push(r),t.push(a)}else{const i=document.createElement("button");i.innerText="Close",i.classList.add("action-button"),i.addEventListener("click",e=>{e.preventDefault(),e.stopPropagation(),shim.hide(!1),this._close(container,callback)}),t.push(i)}const n=document.createElement("div");n.classList.add("button-row"),t.forEach(button=>{n.appendChild(button)}),container.appendChild(n)}}const DateFormats={IsoWithTimezone:"yyyy-mm-ddthh:mm:ssz",IsoWithoutTimezone:"yyyy-mm-ddthh:mm:ss",InternationalWithSeconds:"yyyy-mm-dd hh:mm:ss",International:"yyyy-mm-dd hh:mm",UsDateTimeWithSeconds:"mm/dd/yyyy hh:mm:ss am/pm",UsDateTimeWithoutSeconds:"mm/dd/yyyy hh:mm am/pm",IsoDate:"yyyy-mm-dd",UsDate:"mm/dd/yyyy"},formatMap={"yyyy-mm-ddthh:mm:ssz":format1,"yyyy-mm-ddthh:mm:ss":format2,"yyyy-mm-dd hh:mm:ss":format3,"yyyy-mm-dd hh:mm":format4,"mm/dd/yyyy hh:mm:ss am/pm":format5,"mm/dd/yyyy hh:mm am/pm":format6,"yyyy-mm-dd":format7,"mm/dd/yyyy":format8};
/**
 ...
 * @param {Date} dt
 * @returns {string}
 */function format1(dt){var{year:dt,month:e,day:t,hour:n,minute:r,second:a}=breakDownDate(dt);return`${zeroPad(dt)}-${zeroPad(e)}-${zeroPad(t)}T${zeroPad(n)}:${zeroPad(r)}:${zeroPad(a)}Z`}
/**
 * @param {Date} dt
 * @returns {string}
 */function format2(dt){var{year:dt,month:e,day:t,hour:n,minute:r,second:a}=breakDownDate(dt);return`${zeroPad(dt)}-${zeroPad(e)}-${zeroPad(t)}T${zeroPad(n)}:${zeroPad(r)}:`+zeroPad(a)}
/**
 * @param {Date} dt
 * @returns {string}
 */function format3(dt){var{year:dt,month:e,day:t,hour:n,minute:r,second:a}=breakDownDate(dt);return`${zeroPad(dt)}-${zeroPad(e)}-${zeroPad(t)} ${zeroPad(n)}:${zeroPad(r)}:`+zeroPad(a)}
/**
 * @param {Date} dt
 * @returns {string}
 ...
 * @param {Date} dt
 * @returns {string}
 */function format5(dt){let{year:e,month:t,day:n,hour:r,minute:a,second:i}=breakDownDate(dt),o="AM";return 12<r&&(o="PM",r-=12),0===r&&(r=12),`${zeroPad(t)}/${zeroPad(n)}/${zeroPad(e)} ${zeroPad(r)}:${zeroPad(a)}:${zeroPad(i)} `+o}
/**
 * @param {Date} dt
 * @returns {string}
 */function format6(dt){let{year:e,month:t,day:n,hour:r,minute:a}=breakDownDate(dt),i="AM";return 12<r&&(i="PM",r-=12),0===r&&(r=12),`${zeroPad(t)}/${zeroPad(n)}/${zeroPad(e)} ${zeroPad(r)}:${zeroPad(a)} `+i}
/**
 * @param {Date} dt
 * @returns {string}
...
this.today=new Date,this.inputEl=null,this.popupEl=null,this.headerEl=null,this.bodyEl=null,this.day=0,this.timeSelectorEl=null,this.selectedTimeIndex=0,this.yearBlockStart=this._getYear()-5}connectedCallback(){this.setAttribute("name",this.name+"-datepicker"),this.setAttribute("aria-label","Date Picker"),this.inputEl=this._createInputEl();var e=this._createInputLabel();this.popupEl=this._createPopupEl(),this._drawHeaderEl(),this._drawCalendarBody(),this.insertAdjacentElement("beforeend",this.inputEl),this.insertAdjacentElement("beforeend",e),this.insertAdjacentElement("beforeend",this.popupEl)}
/****************************************************
	 * PUBLIC METHODS
	 ****************************************************/
/**
 ...
 */const debounce=(fn,delay=400)=>{let t=null;return function(){let e=arguments;clearTimeout(t),t=setTimeout(()=>{fn.apply(this,e)},delay)}},objectToMap=(o={})=>{let e=new Map;for(const t in o)e.set(t,o[t]);return e},ErrTokenExpired="token expired";
/**
...
/**
	 ...
	 * @param {object} state The state to pass to the new view.
	 * @returns {void}
	 */navigateTo(url,queryParams={},state={}){let t="";if(0<Object.keys(queryParams).length){var n,r,queryParams=objectToMap(queryParams);t+="?";for([n,r]of queryParams){var a=encodeURIComponent(n);let e=r;"object"==typeof r&&(e=JSON.stringify(r));var i=encodeURIComponent(e);t+=a+`=${i}&`}}history.pushState(state,null,""+url+t),this._route({state:state})}_pathToRegex(path){return new RegExp("^"+path.replace(/\//g,"\\/").replace(/:\w+/g,"(.+)")+"$")}_getParams(match){let e=0;var t=match.result.slice(1),n=Array.from(match.route.path.matchAll(/:(\w+)/g)).map(result=>result[1]);let r={};for(e=0;e<t.length;e++)r[n[e]]=t[e];if(this.injectParams){var a=this.injectParams(match);for(const i in a)r[i]=a[i]}return r}async _route(e){let t={};e.state&&(t=e.state);const n=this.routes.map(route=>({route:route,result:location.pathname.match(this._pathToRegex(route.path))}));let r=n.find(potentialMatch=>null!==potentialMatch.result);
/*
...
/*
		 ...
		 */
e=this._getParams(r);const a=new r.route.view(e),i=(a.state=t,document.querySelector(this.targetEl));i.innerHTML="",i.appendChild(a),this.afterRoute&&this.afterRoute(r.route)}}else this.navigateTo("/404notfound"+location.pathname)}}
