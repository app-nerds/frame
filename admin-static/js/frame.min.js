...
/**
	 ...
	 * @param {function} callback
	 * @returns {void}
	 */show(message,type,callback){const e=document.getElementsByClassName(this.options.position)[0],t=document.createElement("div");t.className="alert-card "+type,t.innerHTML+=svgs[type],t.options={...this.options,message:message,type:type,yPos:-1<this.options.position.indexOf("top")?"top":"bottom",inFocus:!1},this._setContent(t),this._setIntroAnimation(t),this._bindEvents(t),this._autoDestroy(t,callback),e.appendChild(t)}_setContent(card){const e=document.createElement("div");e.className="text-group",card.options.title&&(e.innerHTML=`<h4>${card.options.title}</h3>`),e.innerHTML+=`<p>${card.options.message}</p>`,card.appendChild(e)}
/**
...
/**
	 * @param {AlertCard} card
	 * @returns {void}
	 */_destroy(card,callback){card.style.setProperty("margin-"+card.options.yPos,`-${card.offsetHeight}px`),card.style.setProperty("opacity","0"),setTimeout(()=>{card.remove(),"function"==typeof callback&&callback()},500)}_setup(){const e=document.createElement("div");e.className="alert-container";for(const t of[0,1]){const n=document.createElement("div");n.className="alert-row";for(const r of[0,1,2]){const i=document.createElement("div");i.className="alert-col "+alertPositionIndex[t][r],n.appendChild(i)}e.appendChild(n)}document.body.appendChild(e)}}
...
/**
	 ...
	 * @param {function} callback
	 * @returns {void}
	 */show(type,message,callback){const e=document.createElement("dialog");e.classList.add("confirm-container");let t=new Shim(!0,()=>{this._close(e,callback,!1)});e.innerHTML+=`<p>${message}</p>`,this._addButtons(e,type,t,callback),t.show(),document.body.appendChild(e)}_close(container,callback,callbackValue){container.remove(),"function"==typeof callback&&callback(callbackValue)}_addButtons(container,type,shim,callback){let t=[];if("yesno"===type){const r=document.createElement("button"),i=(r.innerText="No",r.classList.add("cancel-button"),r.addEventListener("click",e=>{e.preventDefault(),e.stopPropagation(),shim.hide(!1),this._close(container,callback,!1)}),document.createElement("button"));i.innerText="Yes",i.classList.add("action-button"),i.addEventListener("click",e=>{e.preventDefault(),e.stopPropagation(),shim.hide(!1),this._close(container,callback,!0)}),t.push(r),t.push(i)}else{const a=document.createElement("button");a.innerText="Close",a.classList.add("action-button"),a.addEventListener("click",e=>{e.preventDefault(),e.stopPropagation(),shim.hide(!1),this._close(container,callback)}),t.push(a)}const n=document.createElement("div");n.classList.add("button-row"),t.forEach(button=>{n.appendChild(button)}),container.appendChild(n)}}const DateFormats={IsoWithTimezone:"yyyy-mm-ddthh:mm:ssz",IsoWithoutTimezone:"yyyy-mm-ddthh:mm:ss",InternationalWithSeconds:"yyyy-mm-dd hh:mm:ss",International:"yyyy-mm-dd hh:mm",UsDateTimeWithSeconds:"mm/dd/yyyy hh:mm:ss am/pm",UsDateTimeWithoutSeconds:"mm/dd/yyyy hh:mm am/pm",IsoDate:"yyyy-mm-dd",UsDate:"mm/dd/yyyy"},formatMap={"yyyy-mm-ddthh:mm:ssz":format1,"yyyy-mm-ddthh:mm:ss":format2,"yyyy-mm-dd hh:mm:ss":format3,"yyyy-mm-dd hh:mm":format4,"mm/dd/yyyy hh:mm:ss am/pm":format5,"mm/dd/yyyy hh:mm am/pm":format6,"yyyy-mm-dd":format7,"mm/dd/yyyy":format8};
/**
 ...
 * @param {Date} dt
 * @returns {string}
 */function format1(dt){var{year:dt,month:e,day:t,hour:n,minute:r,second:i}=breakDownDate(dt);return`${zeroPad(dt)}-${zeroPad(e)}-${zeroPad(t)}T${zeroPad(n)}:${zeroPad(r)}:${zeroPad(i)}Z`}
/**
 * @param {Date} dt
 * @returns {string}
 */function format2(dt){var{year:dt,month:e,day:t,hour:n,minute:r,second:i}=breakDownDate(dt);return`${zeroPad(dt)}-${zeroPad(e)}-${zeroPad(t)}T${zeroPad(n)}:${zeroPad(r)}:`+zeroPad(i)}
/**
 * @param {Date} dt
 * @returns {string}
 */function format3(dt){var{year:dt,month:e,day:t,hour:n,minute:r,second:i}=breakDownDate(dt);return`${zeroPad(dt)}-${zeroPad(e)}-${zeroPad(t)} ${zeroPad(n)}:${zeroPad(r)}:`+zeroPad(i)}
/**
 * @param {Date} dt
 * @returns {string}
 */function format4(dt){var{year:dt,month:e,day:t,hour:n,minute:r}=breakDownDate(dt);return`${zeroPad(dt)}-${zeroPad(e)}-${zeroPad(t)} ${zeroPad(n)}:`+zeroPad(r)}
/**
 * @param {Date} dt
 * @returns {string}
 */function format5(dt){let{year:e,month:t,day:n,hour:r,minute:i,second:a}=breakDownDate(dt),o="AM";return 12<r&&(o="PM",r-=12),0===r&&(r=12),`${zeroPad(t)}/${zeroPad(n)}/${zeroPad(e)} ${zeroPad(r)}:${zeroPad(i)}:${zeroPad(a)} `+o}
/**
 * @param {Date} dt
 * @returns {string}
 */function format6(dt){let{year:e,month:t,day:n,hour:r,minute:i}=breakDownDate(dt),a="AM";return 12<r&&(a="PM",r-=12),0===r&&(r=12),`${zeroPad(t)}/${zeroPad(n)}/${zeroPad(e)} ${zeroPad(r)}:${zeroPad(i)} `+a}
/**
 * @param {Date} dt
 * @returns {string}
...
/(?<month>\d{2})\/(?<day>\d{2})\/(?<year>\d{4})/];for(let e=0;e<t.length;e++){var n=dt.match(t[e]);if(n){var r=parseInt(n.groups.year)||(new Date).getFullYear(),i=parseInt(n.groups.month)||(new Date).getMonth(),a=parseInt(n.groups.day)||(new Date).getDate();let e=parseInt(n.groups.hour)||0;var o=parseInt(n.groups.minute)||0,s=parseInt(n.groups.second)||0;let t=n.groups.meridian||"";return""!==t&&"pm"===t.toLowerCase()&&e<12&&(e+=12),new Date(r,i-1,a,e,o,s)}}throw new Error("no pattern match for "+dt)}
/**
...
/****************************************************
	 * PRIVATE METHODS
	 ****************************************************/_drawHeaderEl(){var e=this._createPreviousMonthButton(),t=this._createNextMonthButton(),n=this._createCurrentMonthButton(),r=this._createCurrentYearButton();this.headerEl.innerHTML="",this.headerEl.insertAdjacentElement("beforeend",e),this.headerEl.insertAdjacentElement("beforeend",n),this.headerEl.insertAdjacentElement("beforeend",r),this.headerEl.insertAdjacentElement("beforeend",t)}_drawCalendarBody(){let t=document.createElement("div"),n=this._createCalendarBodyWeekDiv(),e=this._getFirstDayOfMonth();var r=e.getDay();let i=this._getLastDayOfMonth();var a,o=i.getDate();let s=!1;t.classList.add("calendar-body");for(let e=0;e<o+r;e++){
/*
			 * Basically we want to not render day numbers until we hit the
			 * first day of the month on the correct day of the week.
			 */
s||e===r&&(s=!0);var l=this._createCalendarBodyDayDiv(s,e,r);n.insertAdjacentElement("beforeend",l),
/*
			 * Create a new week div every 7 days.
			 */
(e+1)%7||(t.insertAdjacentElement("beforeend",n),n=this._createCalendarBodyWeekDiv())}""!==n.innerHTML&&t.insertAdjacentElement("beforeend",n),this.showTimeSelector&&(this._createTimeSelector(),t.insertAdjacentElement("beforeend",this.timeSelectorEl),a=this._createOkButton(),t.insertAdjacentElement("beforeend",a)),this._replaceBodyEl(t)}_drawMonthListBody(){const t=document.createElement("div");t.classList.add("month-list-body");for(let e=0;e<this._months.length;e++){var n=this._createMonthButton(e);t.insertAdjacentElement("beforeend",n)}this._replaceBodyEl(t)}_drawYearListBody(){const e=document.createElement("div"),t=(e.classList.add("year-list-body"),document.createElement("div"));t.classList.add("year-list");var n=this._createYearUpButton(),r=this._createYearDownButton();e.insertAdjacentElement("beforeend",n);for(let e=this.yearBlockStart;e<this.yearBlockStart+10;e++){var i=this._createYearButton(e);t.insertAdjacentElement("beforeend",i)}e.insertAdjacentElement("beforeend",t),e.insertAdjacentElement("beforeend",r),this._replaceBodyEl(e)}_getMonth(){return new Date(this.date).getMonth()}_getMonthName(){return this._months[this._getMonth()]}_getYear(){return new Date(this.date).getFullYear()}_getDay(){return new Date(this.date).getDate()}_getFirstDayOfMonth(){return new Date(this._getYear(),this._getMonth(),1)}_getLastDayOfMonth(){return new Date(this._getYear(),this._getMonth()+1,0)}_getHour(){return new Date(this.date).getHours()}_getMinute(){return new Date(this.date).getMinutes()}_getSecond(){return new Date(this.date).getSeconds()}
/**
	 * @param {number} day
...
/**
	 ...
	 */_createCalendarBodyDayDiv(started,dayIndex,firstDayOfWeek){let r=document.createElement("div");if(r.classList.add("day"),started){let t=dayIndex-firstDayOfWeek+1,n=document.createElement("button");n.innerText=""+t,n.setAttribute("type","button"),n.addEventListener("click",e=>this._onCalendarDayClick.call(this,e,t)),new Date(this._getYear(),this._getMonth(),t)===this.today&&n.classList.add("today"),r.insertAdjacentElement("beforeend",n)}else r.classList.add("disabled");return r}_createCalendarBodyWeekDiv(){let e=document.createElement("div");return e.classList.add("week"),e}_createCurrentMonthButton(){let e=document.createElement("button");return e.innerHTML=this._getMonthName(),e.setAttribute("type","button"),e.addEventListener("click",e=>this._onHeaderMonthClick.call(this,e)),e}_createCurrentYearButton(){let e=document.createElement("button");return e.innerHTML=this._getYear().toString(),e.setAttribute("type","button"),e.addEventListener("click",e=>this._onHeaderYearClick.call(this,e)),e}_createInputEl(){let e=document.createElement("input");return e.setAttribute("type","datetime"),e.setAttribute("name",this.name),e.setAttribute("aria-describedby",this.name+"-format"),this.date instanceof Date&&(e.value=formatDateTime(this.date,this.dateFormat)),e.addEventListener("click",()=>{""===this.date&&(this.date=new Date((new Date).getFullYear(),(new Date).getMonth(),(new Date).getDate(),0,0,0),this._drawHeaderEl(),this._drawCalendarBody()),this.toggle()}),e}_createInputLabel(){let e=document.createElement("p");return e.innerText=`(${this.dateFormat})`,e.id=this.name+"-format",e}
/**
	 * @param {number} monthIndex
	 * @returns {HTMLAnchorElement}
	 */_createMonthButton(monthIndex){let t=document.createElement("button");return t.innerText=this._months[monthIndex],t.setAttribute("type","button"),t.addEventListener("click",e=>this._onMonthClick.call(this,e,monthIndex)),t}_createNextMonthButton(){let e=document.createElement("button");return e.innerHTML='<i class="icon--mdi icon--mdi--arrow-right"></i>',e.setAttribute("type","button"),e.addEventListener("click",e=>this.moveMonth.call(this,e,1)),e}_createOkButton(){let e=document.createElement("button");return e.innerText="OK",e.setAttribute("type","button"),e.classList.add("ok"),e.addEventListener("click",this.toggle.bind(this)),e}_createPopupEl(){let e=document.createElement("div");return this.headerEl=document.createElement("header"),this.bodyEl=document.createElement("section"),e.classList.add("date-time-picker-popup","calendar-hidden"),e.setAttribute("role","dialog"),e.setAttribute("aria-modal","true"),e.setAttribute("aria-label","Choose Date"),e.insertAdjacentElement("beforeend",this.headerEl),e.insertAdjacentElement("beforeend",this.bodyEl),e}_createPreviousMonthButton(){let e=document.createElement("button");return e.innerHTML='<i class="icon--mdi icon--mdi--arrow-left"></i>',e.setAttribute("type","button"),e.addEventListener("click",e=>this.moveMonth.call(this,e,-1)),e}_createTimeSelector(){this.timeSelectorEl=document.createElement("select"),this._createTimeSelectorOptions(),this.timeSelectorEl.addEventListener("change",this._onTimeChange.bind(this))}_createTimeSelectorOptions(){this.timeSelectorEl.innerHTML="";let t=1,r=("5minute"===this.timeIncrement&&(t=5),"10minute"===this.timeIncrement&&(t=10),"15minute"===this.timeIncrement&&(t=15),"30minute"===this.timeIncrement&&(t=30),"hour"===this.timeIncrement&&(t=60),new Date(this._getYear(),this._getMonth(),this._getDay(),0,0,0)),i=0;for(let e=0;e<1440;e+=t){let n=document.createElement("option");if(n.value=formatDateTime(r,this.dateFormat),i===this.selectedTimeIndex&&n.setAttribute("selected","selected"),this.twentyFourHourTime)n.innerText=r.getHours().toString().padStart(2,"0")+":"+r.getMinutes().toString().padStart(2,"0");else{let e=r.getHours(),t="AM";12<e&&(e-=12,t="PM"),12===e&&(t="PM"),0===e&&(e=12),n.innerText=`${e.toString().padStart(2,"0")}:${r.getMinutes().toString().padStart(2,"0")} `+t}this.timeSelectorEl.insertAdjacentElement("beforeend",n),r=new Date(r.getTime()+6e4*t),i++}}
/**
	 * @param {number} year
	 * @returns {HTMLAnchorElement}
	 */_createYearButton(year){let t=document.createElement("button");return t.innerText=year.toString(),t.setAttribute("type","button"),t.addEventListener("click",e=>this._onYearClick.call(this,e,year)),t}_createYearDownButton(){const e=document.createElement("button");return e.innerHTML='<i class="icon--mdi icon--mdi--arrow-down"></i>',e.setAttribute("type","button"),e.addEventListener("click",e=>this._onYearDownClick.call(this,e)),e}_createYearUpButton(){const e=document.createElement("button");return e.innerHTML='<i class="icon--mdi icon--mdi--arrow-up"></i>',e.setAttribute("type","button"),e.addEventListener("click",e=>this._onYearUpClick.call(this,e)),e}}customElements.define("date-time-picker",DateTimePicker);
/**
 ...
 * @extends HTMLElement
 */
class PopupMenu extends HTMLElement{constructor(){super(),this._trigger=null,this.isVisible=!1}connectedCallback(){if(this._trigger=this.getAttribute("trigger"),!this._trigger)throw new Error("You must provide a query selector for the element used to trigger this popup.");this.style.visibility="hidden",document.addEventListener("click",e=>{e.target===this||this.contains(e.target)||(e.target!==document.querySelector(this._trigger)?this._hide():this.toggle())});const e=document.querySelectorAll("popup-menu-item");e.forEach(el=>{el.addEventListener("internal-menu-item-click",e=>{this._hide(),this.dispatchEvent(new CustomEvent("menu-item-click",{detail:{id:e.target.id,text:e.target.getAttribute("text"),data:e.target.getAttribute("data")}}))})})}disconnectedCallback(){let e=document.querySelector(this._trigger);e&&e.removeEventListener("click",this.toggle.bind(this))}
/**
	* Toggles the visibility of the popup menu
	* @param {Event} e The click event
	* @returns {void}
	*/toggle(e){e&&e.preventDefault(),this.isVisible?this._hide():this._show()}_hide(){this.isVisible=!1,this.style.visibility="hidden"}_show(){var e=document.querySelector(this._trigger).getBoundingClientRect(),t=this.getBoundingClientRect();t.right>window.innerWidth?this.style.left=e.x+(window.innerWidth-t.right)-3+"px":this.style.left=e.x+"px",this.style.top=e.y+e.height+3+"px",this.isVisible=!0,this.style.visibility="visible"}}
/**
 * Represents a popup menu item
 * @class PopupMenuItem
 * @extends HTMLElement
 */class PopupMenuItem extends HTMLElement{constructor(){super()}connectedCallback(){this._render()}_render(){var e=this.getAttribute("text"),t=this.getAttribute("icon");const n=document.createElement("a");n.href="javascript:void(0)",n.classList.add("popup-menu-item");let r="";t&&(r+=`<i class="${t}"></i> `),r+=e,n.innerHTML=r,n.addEventListener("click",e=>{e.preventDefault(),e.stopPropagation(),this.dispatchEvent(new CustomEvent("internal-menu-item-click",{detail:e}))}),this.insertAdjacentElement("beforeend",n)}}
/**
 * Shows a popup menu
 * @param {string} el The query selector for the popup menu
 * @returns {void}
 */const showPopup=el=>{document.querySelector(el).style.visibility="visible"},hidePopup=el=>{document.querySelector(el).style.visibility="hidden"};
/**
 * Hides a popup menu
 * @param {string} el The query selector for the popup menu
...
/**
	 ...
	 * @param {object} state The state to pass to the new view.
	 * @returns {void}
	 */navigateTo(url,queryParams={},state={}){let t="";if(0<Object.keys(queryParams).length){var n,r,queryParams=objectToMap(queryParams);t+="?";for([n,r]of queryParams){var i=encodeURIComponent(n);let e=r;"object"==typeof r&&(e=JSON.stringify(r));var a=encodeURIComponent(e);t+=i+`=${a}&`}}history.pushState(state,null,""+url+t),this._route({state:state})}_pathToRegex(path){return new RegExp("^"+path.replace(/\//g,"\\/").replace(/:\w+/g,"(.+)")+"$")}_getParams(match){let e=0;var t=match.result.slice(1),n=Array.from(match.route.path.matchAll(/:(\w+)/g)).map(result=>result[1]);let r={};for(e=0;e<t.length;e++)r[n[e]]=t[e];if(this.injectParams){var i=this.injectParams(match);for(const a in i)r[a]=i[a]}return r}async _route(e){let t={};e.state&&(t=e.state);const n=this.routes.map(route=>({route:route,result:location.pathname.match(this._pathToRegex(route.path))}));let r=n.find(potentialMatch=>null!==potentialMatch.result);
/*
		 * Route not found - return first route
		 */if(r){if(!this.beforeRoute||!1!==this.beforeRoute.apply(this,r.route)){
/*
		 * Get parameters, then initialie the view and render.
		 */
e=this._getParams(r);const i=new r.route.view(e),a=(i.state=t,document.querySelector(this.targetEl));a.innerHTML="",a.appendChild(i),this.afterRoute&&this.afterRoute(r.route)}}else this.navigateTo("/404notfound"+location.pathname)}}
